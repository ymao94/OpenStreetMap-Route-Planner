...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
    //return sqrt(pow(node->x - end_node->x, 2) + pow(node->y - end_node->y, 2));
...
// static bool RoutePlanner::Compare(RouteModel::Node *node1, RouteModel::Node *node2) {
...
void RoutePlanner::AStarSearch() {
    ...
    current_node->visited = true; // Aha! I have only forgotten this.
    while (current_node != this->end_node) {
        AddNeighbors(current_node);
        // shouldn't we clear the open_list after each step?
